import type { DbmlDocument, DbmlReference, DbmlTable } from "@/features/modeling-dashboard/modeling-workspace/model-document/dbml/types";
import type { DataModelValidationIssueResponse } from "@/api";
import { groupValidationIssues } from "@/features/modeling-dashboard/validation/utils/group-validation-issues";
import type {
  ERDRelationshipEdge,
  ERDTableNode,
...
export function mapDocumentToGraph(
  ...
  const issuesByTable = groupValidationIssues(validationIssues);
  return {
    nodes: document.tables.map<ERDTableNode>((table, index) => ({
      ...
      data: {
        ...
        validationIssues: issuesByTable.get(table.name.toLocaleLowerCase()) ?? [],
      },
    })),
