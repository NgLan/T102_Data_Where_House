...
describe("DataModelInspector", () => {
  it("hiển thị table đang chọn", () => {
    ...
    expect(
      screen.getByDisplayValue(document.tables[0].name),
    ).toBeInTheDocument();
    expect(
      screen.getAllByRole("combobox", { name: "DATA_TYPE_LABEL" }),
    ).not.toHaveLength(0);
  });
  ...
  it("đổi cardinality relationship qua reducer action", () => {
    ...
    render(
      <DataModelInspector
        ...
        selectedReferenceId={document.references[0].id}
        mutate={mutate}
        onAddTable={vi.fn()}
        onAddColumn={vi.fn()}
        onClearSelection={vi.fn()}
    ...
    fireEvent.change(screen.getByLabelText("RELATIONSHIP_KIND_LABEL"), {
      target: { value: "one-to-one" },
    });
    expect(mutate).toHaveBeenCalledWith(
      expect.objectContaining({
        type: "update-reference",
        reference: expect.objectContaining({ relation: "-" }),
      }),
    );
  });
  ...
  it("hiển thị validation cạnh field và cập nhật checkbox qua reducer", () => {
    ...
    render(
      <DataModelInspector
        document={document}
        validationErrors={{ "tables.0.name": "MSG_INVALID_IDENTIFIER" }}
        selectedTableId={document.tables[0].id}
        selectedReferenceId={null}
        mutate={mutate}
        onAddTable={vi.fn()}
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: /BTN_COLUMN_SETTINGS/ })[0],
    );
