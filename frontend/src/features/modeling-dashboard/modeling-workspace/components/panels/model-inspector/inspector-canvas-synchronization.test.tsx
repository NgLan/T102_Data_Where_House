...
describe("inspector và canvas synchronization", () => {
  it("cập nhật node canvas khi sửa select, checkbox và tên trong inspector", () => {
    ...
    fireEvent.change(
      screen.getAllByRole("combobox", { name: "DATA_TYPE_LABEL" })[0],
      { target: { value: "bigint" } },
    );
    ...
    fireEvent.change(screen.getByLabelText("TABLE_NAME_LABEL"), {
      target: { value: "rides_v2" },
    });
    expect(screen.getByTestId("canvas-table-name")).toHaveTextContent(
      "rides_v2",
    );
  });
  ...
  it("dùng input số nguyên và báo lỗi default sai kiểu", () => {
    ...
    const defaultInput = screen.getByRole("spinbutton", {
      name: "DEFAULT_LABEL",
    });
...
function expandColumnSettings(index: number) {
  fireEvent.click(
    screen.getAllByRole("button", { name: /BTN_COLUMN_SETTINGS/ })[index],
  );
}

function SynchronizationHarness() {
  const [document, mutate] = useReducer(
  ...
  return (
    <>
      ...
      <output data-testid="canvas-data-type">
        {table.columns[0].dataType}
      </output>
      <output data-testid="canvas-primary-key">
        {String(table.columns[0].isPrimaryKey)}
      </output>
      <DataModelInspector
        document={document}
        validationErrors={validateDataModel(document)}
        selectedTableId={document.tables[0].id}
