...
export function readDbmlLibraryNote(note: TableModel['note']): string {
  if (typeof note === 'string') return note;
  return note?.value ?? '';
}

export function formatDbmlLibraryDefault(value?: FieldModel['dbdefault']): string {
  if (!value) return '';
...
export function parseDefaultToDbmlLibraryModel(
  value: string,
): FieldModel['dbdefault'] | undefined {
