...
export const dbmlColumnSchema = z.object({
  id: z.string().min(1),
  name: z.string().trim().min(1, 'MSG_FIELD_REQUIRED').regex(IDENTIFIER_PATTERN, 'MSG_INVALID_IDENTIFIER'),
  dataType: z.string().trim()
    .refine(isValidDbmlDataType, 'MSG_INVALID_DATA_TYPE')
    .refine(hasValidDbmlTypeParameters, 'MSG_MSG_INVALID_DATA_TYPE_PARAMETERS'),
  isPrimaryKey: z.boolean(),
  isNotNull: z.boolean(),
  isUnique: z.boolean(),
  isAutoIncrement: z.boolean(),
  defaultValue: z.string().refine(hasBalancedQuotes, 'MSG_INVALID_DEFAULT_VALUE'),
  note: z.string(),
  checks: z.array(z.string().trim().min(1, 'MSG_INVALID_CHECK_EXPRESSION')),
  ...
  if (!isDbmlDefaultValueCompatible(column.dataType, column.defaultValue)) {
    context.addIssue({ code: 'custom', message: 'MSG_INVALID_DEFAULT_FOR_DATA_TYPE', path: ['defaultValue'] });
  }
  if (column.isAutoIncrement && !isIntegerDbmlType(column.dataType)) {
    context.addIssue({ code: 'custom', message: 'MSG_INVALID_INCREMENT_DATA_TYPE', path: ['isAutoIncrement'] });
  }
  if (column.isAutoIncrement && column.defaultValue.trim()) {
    context.addIssue({ code: 'custom', message: 'MSG_INCREMENT_DEFAULT_CONFLICT', path: ['defaultValue'] });
  }
  if ((column.isNotNull || column.isPrimaryKey)
    && column.defaultValue.trim().toLowerCase() === 'null') {
    context.addIssue({ code: 'custom', message: 'MSG_NOT_NULL_DEFAULT_CONFLICT', path: ['defaultValue'] });
  }
});
...
function addDuplicateIssues(
  ...
  names.forEach((name, index) => {
    ...
    if (seen.has(normalized)) {
      context.addIssue({
        code: 'custom',
        message: 'MSG_DUPLICATE_IDENTIFIER',
