...
import { formatDefault, noteValue, parseDefault } from "./dbml-default-mapper";
import {
  emptyFieldModel,
  type FieldModel,
  type TableModel,
...
export function mapTable(table: TableModel, tableIndex: number): DbmlTable {
  return {
    id: tableId(tableIndex),
    schemaName: table.schemaName ?? "public",
    name: table.name,
    note: noteValue(table.note),
...
export function mergeTable(source: TableModel, view: DbmlTable): TableModel {
...
function mergeField(source: FieldModel, view: DbmlColumn): FieldModel {
  return {
    ...
    note: view.note ? { value: view.note } : undefined,
    dbdefault: parseDefault(view.defaultValue),
