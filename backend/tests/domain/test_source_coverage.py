...
def test_unknown_requires_candidate_but_missing_forbids_candidate() -> None:
    ...
    with pytest.raises(BusinessException):
        SourceCoverageAssessment(
            id=uuid4(), batch_id=uuid4(), evaluated_source_revision=1,
            status=SourceCoverageStatus.NEEDS_SOURCE_CONFIRMATION,
            required_concept_key="PATIENT_IDENTITY", title="Identify a patient",
            explanation="Count once.", question="Which field?",
        )
    with pytest.raises(BusinessException):
        SourceCoverageAssessment(
            id=uuid4(), batch_id=uuid4(), evaluated_source_revision=1,
            status=SourceCoverageStatus.MISSING_SOURCE,
            required_concept_key="PATIENT_IDENTITY", title="Missing identity",
            explanation="No identity exists.", candidates=(candidate,),
        )
...
def test_confirmation_resolution_is_item_scoped_and_revisioned() -> None:
    ...
    assessment = SourceCoverageAssessment(
        ...
        status=SourceCoverageStatus.NEEDS_SOURCE_CONFIRMATION,
        required_concept_key="PATIENT_IDENTITY", title="Identify a patient",
        explanation="Count once.", question="Which field?", candidates=(candidate,),
    )
...
def _schema() -> SchemaMetadata:
    column = ColumnMetadata(
        "record_no", ColumnDataType.TEXT, distinct_count=10, is_unique_candidate=True
    )
...
def _source() -> DataSource:
    return DataSource(
        ...
        type=DataSourceType.CSV, schema_metadata=_schema(),
    )
