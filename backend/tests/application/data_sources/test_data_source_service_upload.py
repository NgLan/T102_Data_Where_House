...
async def test_upload_returns_pending_without_profiling() -> None:
    ...
    sources, files, validator = MagicMock(), MagicMock(), MagicMock()
    ...
    service = DataSourceService(sources, files, validator, MagicMock(), unit_of_work, access, projects)

    result = await service.upload_data_sources(
        UploadDataSourcesInput(project.id, (UploadFileInput("orders.csv", b"id\n1"),))
    )

    validator.validate.assert_called_once_with(b"id\n1", "orders.csv")
