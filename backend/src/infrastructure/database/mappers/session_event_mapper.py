...
class SessionEventMapper:
    ...
    def metadata_to_dict(metadata: SessionEventMetadata | None) -> dict[str, Any] | None:
        ...
        if isinstance(metadata, AgentCallMetadata):
            return {
                "caller_agent": str(
                    metadata.caller_agent.value
                    if hasattr(metadata.caller_agent, "value")
                    else metadata.caller_agent
                ),
                "target_agent": str(
                    metadata.target_agent.value
                    if hasattr(metadata.target_agent, "value")
                    else metadata.target_agent
                ),
        ...
        if isinstance(metadata, AgentResultMetadata):
            ...
            return {
                ...
                "agent": str(
                    metadata.agent.value if hasattr(metadata.agent, "value") else metadata.agent
                ),
                "status": str(
    ...
    def dict_to_metadata(
        data: dict[str, Any] | None, event_type: SessionEventType
