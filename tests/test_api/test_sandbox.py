...
from src.presentation.dependencies.sandbox import (
    get_execute_ddl_service,
    get_sandbox_config_service,
)
...
async def test_get_missing_sandbox_config_returns_null(client) -> None:
    service = AsyncMock(spec=ISandboxConfigService)
    service.get_config.return_value = None
    app.dependency_overrides[get_sandbox_config_service] = lambda: service
    try:
...
async def test_save_sandbox_config_contract(client) -> None:
    service = AsyncMock(spec=ISandboxConfigService)
    service.save_config.return_value = SandboxConfigResponse(
        id=uuid4(),
...
async def test_execute_sandbox_ddl_failure_contract(client) -> None:
    service = AsyncMock(spec=IExecuteDdlService)
    service.execute_ddl.return_value = ExecuteDdlResponse(
        success=False,
    ...
    app.dependency_overrides[get_execute_ddl_service] = lambda: service
    try:
        response = await client.post(
